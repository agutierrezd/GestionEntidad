...
function checkTableName($shortTName )
{
	...
	if ("global_status" == $shortTName )
		return true;
	if ("dependencia_det" == $shortTName )
		return true;
	if ("dependencia_det_dir" == $shortTName )
		return true;
	...
	if ("admin_users" == $shortTName )
		return true;
	if ("q_graph_001_chart" == $shortTName )
		return true;
	if ("q_graph_002_chart" == $shortTName )
		return true;
	if ("008_reserva" == $shortTName )
		return true;
	return false;
}
...
function GetTablesList($pdfMode = false)
{
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("q_graph_001 Chart");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="q_graph_001 Chart";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("q_graph_002 Chart");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="q_graph_002 Chart";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("008_reserva");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
...
{
	if($shorttable=="q_graph_001_chart")
		return "2DColumn";
	if($shorttable=="q_graph_002_chart")
		return "2DColumn";
	return "";
}
...
//	default permissions
	...
	if( $table=="q_graph_001 Chart" )
	{
//	default permissions
		return "S".$extraPerm;
	}
	if( $table=="q_graph_002 Chart" )
	{
//	default permissions
		return "S".$extraPerm;
	}
	if( $table=="008_reserva" )
	{
//	default permissions
		return "ADESPI".$extraPerm;
	}
...
function GetBaseScriptsForPage($isDisplayLoading, $additionalScripts = "", $customText = "")
{
	...
	$result = "";
	$result .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/loadfirst.js?38512")."\"></script>";


	$result .= $additionalScripts;
	$result .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/lang/".getLangFileName(mlang_getcurrentlang()).".js?38512")."\"></script>";
